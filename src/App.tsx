...
import { analyzeMove, type CoachLanguage, type CoachResult, } from './coach';
import { speakCoach, stopCoachSpeech, unlockCoachAudio, } from './tts';
import { abortGame, challengeBot, getAccount, getPlayingGames, handleTakeback, LichessHttpError, makeMove, resignGame, retryingStream, streamEvents, streamGame, type Account, type StreamEvent, } from './lichess';
...
type CoachNote = CoachResult & {
    gameId: string;
    savedAt: number;
    playerColor: 'white' | 'black';
    language?: CoachLanguage;
};
type ReviewTarget = CoachResult & {
    playerColor?: 'white' | 'black';
};
...
type PuzzleState = 'solving' | 'incorrect' | 'correct' | 'revealed';
const GAME_EVALUATION_STORAGE_KEY = 'ai-chess-coach.game-evaluations.v1';
function readEvaluationSessions(): StoredEvaluationSession[] {
    try {
        const raw = window.localStorage.getItem(GAME_EVALUATION_STORAGE_KEY);
        if (!raw)
            return [];
        const parsed = JSON.parse(raw);
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter((session): session is StoredEvaluationSession => Boolean(session &&
...
function readGameEvaluations(gameId: string): CoachResult[] {
    return (readEvaluationSessions().find((session) => session.gameId === gameId)?.evaluations || []);
}
function storeGameEvaluations(gameId: string, username: string | undefined, evaluations: CoachResult[]) {
    try {
        const existing = readEvaluationSessions().filter((session) => session.gameId !== gameId);
        const next: StoredEvaluationSession[] = [
            {
                gameId,
                username,
                updatedAt: Date.now(),
                evaluations: evaluations.slice(-160),
            },
            ...existing,
        ].slice(0, 8);
        window.localStorage.setItem(GAME_EVALUATION_STORAGE_KEY, JSON.stringify(next));
    }
    catch {
        // localStorage may be unavailable.
    }
}
function classificationWeight(classification: CoachResult['classification']): number {
...
function selectPracticePuzzles(notes: CoachNote[], limit = 3): CoachNote[] {
    const ranked = [...notes]
        .filter((note) => Boolean(note.fenBefore) &&
        Boolean(note.bestMoveUci) &&
        note.bestMoveUci.length >= 4 &&
        (note.bestMoveUci.length === 4 || note.bestMoveUci[4] === 'q'))
        .sort((a, b) => {
        const scoreA = classificationWeight(a.classification) +
            a.centipawnLoss;
        const scoreB = classificationWeight(b.classification) +
            b.centipawnLoss;
    ...
    for (const note of ranked) {
        if (selected.some((existing) => existing.ply === note.ply)) {
            continue;
        }
        selected.push(note);
        if (selected.length >= limit) {
...
function buildGameReport(evaluations: CoachResult[], notes: CoachNote[]): GameReport {
    const ordered = [...evaluations].sort((a, b) => a.ply - b.ply);
    const goodMoves = ordered.filter((result) => result.classification === 'good');
    const accurateMoves = [...goodMoves]
        .sort((a, b) => a.centipawnLoss - b.centipawnLoss)
        .slice(0, 2);
    const strengths: string[] = [];
    for (const result of accurateMoves) {
        strengths.push(`Move ${result.moveNumber}: ${result.playedMove} was one of your strongest decisions (${result.centipawnLoss} cp loss).`);
    }
    if (goodMoves.length > 0 &&
        strengths.length < 2) {
        strengths.push(`${goodMoves.length} of your analyzed moves were classified as good.`);
    }
    if (!strengths.length) {
        strengths.push('Keep looking for forcing moves and comparing your candidate moves before committing.');
    }
    ...
    for (const note of notes) {
        ...
        if (current) {
            current.count += 1;
            current.worst = Math.max(current.worst, note.centipawnLoss);
        }
        else {
            themes.set(key, {
                text: lesson,
                count: 1,
                worst: note.centipawnLoss,
            });
        }
    }
    const improvementThemes = [
        ...
        .sort((a, b) => b.count * 100 +
        b.worst -
        (a.count * 100 + a.worst))
        .slice(0, 2);
    const improvements = improvementThemes.map((theme) => theme.count > 1
        ? `${theme.text} This came up ${theme.count} times in the game.`
        : theme.text);
    if (!improvements.length && notes.length) {
        const worst = [...notes].sort((a, b) => b.centipawnLoss - a.centipawnLoss)[0];
        improvements.push(`Revisit move ${worst.moveNumber} (${worst.playedMove}) and compare it with ${worst.bestMove}.`);
    }
    if (!improvements.length) {
        improvements.push('No major recurring weakness stood out in the analyzed moves.');
    }
    const mainTheme = improvementThemes[0]?.text;
    let takeaway = 'Keep using the same thinking process and try to make your good decisions more consistent.';
    if (mainTheme) {
        takeaway =
            `Your main practice focus from this game should be: ${mainTheme}`;
    }
    else if (!notes.length && evaluations.length) {
        takeaway =
            'This was a clean game. Focus on repeating the decision-making process that produced your strongest moves.';
    }
    return {
...
const COACH_LANGUAGE_STORAGE_KEY = 'ai-chess-coach.coach-language.v1';
...
function readLearningSessions(): StoredLearningSession[] {
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter((session): session is StoredLearningSession => Boolean(session && typeof session.gameId === 'string' && Array.isArray(session.notes)));
    }
    catch {
        return [];
    }
...
function storePreferredTimeControl(value: TimeControlId) {
    try {
        window.localStorage.setItem(TIME_CONTROL_STORAGE_KEY, value);
    }
    catch { /* no-op */ }
}
...
function readCoachLanguage(): CoachLanguage {
    try {
        const value = window.localStorage.getItem(COACH_LANGUAGE_STORAGE_KEY);
        return value === 'zh-CN'
            ? 'zh-CN'
            : 'en';
    }
...
function replay(initialFen: string, movesText: string) {
    ...
    return {
        ...
        lastMove: last ? [last.slice(0, 2), last.slice(2, 4)] as [
            string,
            string
        ] : undefined,
    };
...
function PlayerBar({ player, clock, active, side }: {
    player: Player;
    clock: number | null;
    active: boolean;
    side: 'white' | 'black';
...
export default function App() {
    ...
    const displayPosition = useMemo(() => {
        ...
        const moves = movesText.trim()
            ...
            : [];
        return replay(initialFen, moves.slice(0, historyPly).join(' '));
    ...
    const isSenseRobotGame = Boolean(gameId && senseRobotGameId === gameId);
    const isMyTurn = activeGame && myColor === turnColor;
    const canMove = !isSenseRobotGame &&
        historyPly == null &&
        isMyTurn &&
        !moveInFlight &&
        !pendingPromotion;
    ...
    const coachHighlights = useMemo(() => {
        if (!hintsEnabled || !coachResult)
            return [];
        if (position.plyCount === coachResult.ply - 1)
            return coachResult.highlightsBefore || [];
        if (position.plyCount === coachResult.ply)
            return coachResult.highlightsAfter || [];
        return [];
    }, [hintsEnabled, coachResult, position.plyCount]);
    ...
    useEffect(() => {
        try {
            window.localStorage.setItem(COACH_DETAIL_STORAGE_KEY, coachDetail);
        }
        catch {
            // no-op
        }
    ...
    useEffect(() => {
        try {
            window.localStorage.setItem(COACH_LANGUAGE_STORAGE_KEY, coachLanguage);
        }
        catch {
            // no-op
        }
    ...
    useEffect(() => {
        ...
        getPlayingGames(token).then((games) => {
            if (cancelled || gameIdRef.current)
                return;
            ...
            if (!cancelled)
                setStatus(`Could not check active games yet: ${String(error)}`);
        }).finally(() => {
            if (!cancelled)
                setRecoveryChecked(true);
        });
    ...
    useEffect(() => {
        ...
        const refresh = async () => {
            try {
                ...
                if (alive)
                    setBot({ running: false, connected: false, error: String(error) });
            }
        };
        void startBot().then((state) => { if (alive)
            setBot(state); }).catch((error) => {
            if (alive)
                setBot({ running: false, connected: false, error: String(error) });
        });
        void refresh();
    ...
    useEffect(() => {
        ...
        void retryingStream((signal) => streamEvents(token, (event: StreamEvent) => {
            if (event.type === 'gameStart' && event.game) {
                ...
                if (opponent === BOT_USERNAME.toLowerCase()) {
                    ...
                    setSenseRobotGameId((current) => current === startedGameId ? current : null);
        ...
        }, signal), controller.signal, () => setStatus('Reconnecting to Lichess…'));
        return () => controller.abort();
    }, [token, account, setActiveGameId]);
    useEffect(() => {
        if (!token || !gameId || !account)
            return;
        const controller = new AbortController();
        const syncPendingMove = (nextMovesText: string) => {
            ...
            if (moves.length <= pendingMove.basePly)
                return;
            pendingMoveRef.current = null;
            setMoveInFlight(false);
        ...
        const applyState = (state: any, clockEnabled?: boolean) => {
            ...
            const nextWinner: Winner = state?.winner === 'white' || state?.winner === 'black'
                ? state.winner
                : null;
        ...
        void retryingStream((signal) => streamGame(token, gameId, onGameEvent, signal), controller.signal, () => setStatus('Reconnecting to the game…'), (error) => {
            setRecoveryChecked(true);
    ...
    useEffect(() => {
        ...
        const syncFromBotBackend = async () => {
            ...
            try {
                ...
                const cachedMoves = String(cached.state?.moves || '').trim();
                const currentMoves = movesTextRef.current.trim();
                ...
                if (cachedMoveList.length >
                    ...
                    setClock((previous) => ({
                        ...
                        white: typeof state.wtime === 'number'
                            ? state.wtime
                            : previous.white,
                        black: typeof state.btime === 'number'
                            ? state.btime
                            : previous.black,
                        increment: typeof state.winc === 'number'
                            ? state.winc
                            : previous.increment,
                        updatedAt: Date.now(),
                    }));
                    ...
                    setStatus('Game resynced automatically.');
                }
            }
            catch (error) {
                ...
                console.warn('Backup game sync failed:', error);
            }
            finally {
        ...
        const timer = window.setInterval(() => void syncFromBotBackend(), 1500);
        return () => {
            cancelled = true;
    ...
    useEffect(() => {
        ...
        if (gameId) {
            setCoachNotes(readLearningNotes(gameId));
            setMoveEvaluations(readGameEvaluations(gameId));
        }
        else {
            setCoachNotes([]);
            setMoveEvaluations([]);
    ...
    useEffect(() => {
        ...
        storeGameEvaluations(gameId, account?.username, moveEvaluations);
    }, [
        gameId,
        account?.username,
        moveEvaluations,
    ...
    function speak(text: string) {
        if (!voiceEnabled)
            return;
        void speakCoach(text, coachLanguage);
    }
    const processCoachQueue = useCallback(async () => {
        if (coachProcessingRef.current)
            return;
        ...
        try {
            while (coachQueueRef.current.length > 0) {
                ...
                setCoachError('');
                try {
                    const result = await analyzeMove(job.fenBefore, job.uci, coachDetail, controller.signal, coachLanguage);
                    setMoveEvaluations((current) => {
                        const next = [
                            ...current.filter((item) => item.ply !== result.ply),
                            result,
                        ];
    ...
    useEffect(() => {
        ...
        // etc.
        for (let moveIndex = previousPly; moveIndex < currentPly; moveIndex += 1) {
            const moverColor: 'white' | 'black' = moveIndex % 2 === 0
                ? 'white'
                : 'black';
        ...
        const movesBeforeStudentMove = moves
            ...
            .join(' ');
        const beforePosition = replay(initialFen, movesBeforeStudentMove);
        analyzeStudentMove(beforePosition.chess.fen(), uci);
    }, [
        gameId,
        movesText,
        initialFen,
    ...
    const submitMove = useCallback(async (fenBefore: string, basePly: number, from: string, to: string, promotion?: PromotionChoice) => {
    ...
    const handleBoardMove = useCallback((from: string, to: string) => {
        ...
        const promotions = Array.from(new Set(legal.map((move) => move.promotion).filter(Boolean) as PromotionChoice[]));
    ...
    async function waitForBotReady(): Promise<BotRuntimeStatus> {
        ...
        while (Date.now() < deadline) {
            ...
            setBot(last);
            if (last.running && last.connected)
                return last;
    ...
    function changeTimeControl(nextId: TimeControlId) {
        ...
        if (selected)
            setStatus(`Time control: ${selected.label} · ${selected.detail}.`);
    }
    async function startCoachGame() {
        if (!token || !account || startingGame || activeGame || gameId || !recoveryChecked)
            return;
        ...
        try {
            ...
            const existing = existingGames.find((game) => game.opponent?.username?.toLowerCase() ===
            ...
            const selectedTimeControl = TIME_CONTROLS.find((item) => item.id === timeControlId) ||
                TIME_CONTROLS[4];
            ...
            if (!acceptedGameId) {
                ...
                while (Date.now() < deadline && !acceptedGameId) {
                    ...
                    try {
                        ...
                        const found = games.find((game) => game.opponent?.username?.toLowerCase() ===
                            BOT_USERNAME.toLowerCase());
                        if (found?.gameId) {
            ...
            if (!acceptedGameId) {
                throw (acceptError ||
                    new Error('The challenge was accepted, but the game did not become visible in time.'));
            }
            ...
            setStatus(`Could not start game: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
    ...
    async function scanSenseRobotRoom() {
        ...
        try {
            ...
            setBot(joined);
            if (!joined.gameId) {
                throw new Error('The room was joined, but Lichess did not report a game.');
            }
            ...
            setGameStatus('recovering');
            setStatus(`SenseRobot room joined as ${BOT_USERNAME}.`);
        }
        catch (error) {
            setStatus(`Could not join SenseRobot room: ${error instanceof Error
                ? error.message
                : String(error)}`);
        }
        finally {
    ...
    async function endCurrentGame() {
        if (!token || !gameId || !activeGame || !endGameConfirm)
            return;
        try {
            setEndGameConfirm(false);
    ...
    function resetFinishedGame() {
        ...
        setPuzzleRollbackSignal((value) => value + 1);
    ...
    const goodMoves = moveEvaluations.filter((result) => result.classification === 'good').length;
    const practicePuzzles = selectPracticePuzzles(coachNotes);
    const gameReport = buildGameReport(moveEvaluations, coachNotes);
    const activePuzzle = practicePuzzles[puzzleIndex] || null;
    const puzzleChess = activePuzzle
        ? new Chess(puzzleFen ||
            activePuzzle.fenBefore)
        : null;
    const puzzleDestinations = puzzleChess
        ? destinations(puzzleChess)
        : new Map<string, string[]>();
    const puzzleMovableColor: 'white' | 'black' | undefined = puzzleChess
        ? puzzleChess.turn() === 'w'
            ? 'white'
            : 'black'
        : undefined;
    ...
    const reviewIndex = reviewTarget
        ? orderedCoachNotes.findIndex((note) => note.ply === reviewTarget.ply)
        : -1;
    function openPuzzle(index: number) {
        const puzzle = practicePuzzles[index];
        if (!puzzle)
            return;
    ...
    function handlePuzzleMove(from: string, to: string) {
        ...
        const answerUci = activePuzzle.bestMoveUci.toLowerCase();
        // startsWith also handles promotion moves such as e7e8q.
        if (answerUci.startsWith(playedUci)) {
            const chess = new Chess(activePuzzle.fenBefore);
            chess.move({
                from,
                to,
                promotion: answerUci[4] || 'q',
            });
    ...
    function revealPuzzleAnswer() {
        if (!activePuzzle)
            return;
        setPuzzleFen(activePuzzle.fenBefore);
    ...
    function openReviewAt(index: number) {
        if (!orderedCoachNotes.length)
            return;
        const normalized = (index + orderedCoachNotes.length) % orderedCoachNotes.length;
        setReviewTarget(orderedCoachNotes[normalized]);
    }
    function previousReview() {
        if (!orderedCoachNotes.length)
            return;
        openReviewAt(reviewIndex >= 0
            ? reviewIndex - 1
    ...
    <main className="game-layout">
      <section className="play-column">
        ...
        <section className="board-panel">
          <ChessBoard fen={displayPosition.chess.fen()} orientation={orientation} movableColor={canMove ? myColor : undefined} destinations={canMove ? destinations(position.chess) : new Map()} lastMove={displayPosition.lastMove} coachArrows={historyPly == null ? coachArrows : []} coachHighlights={historyPly == null ? coachHighlights : []} rollbackSignal={rollbackSignal} onMove={handleBoardMove}/>
        </section>
        ...
        <div className="under-board">
          <span>
            {gameId ? (<span className="under-board-meta">
      ...
      <aside className="side-panel">
        {!gameId && <section className="card setup-card">
          ...
          <div className="level-grid">
            {LEVELS.map((item) => <button key={item.id} className={level === item.id ? 'level active' : 'level'} onClick={() => void changeLevel(item.id)}>
          ...
          <div className="time-control-grid" role="group" aria-label="Choose time control">
            {TIME_CONTROLS.map((item) => <button key={item.id} className={timeControlId === item.id ? 'time-control active' : 'time-control'} onClick={() => changeTimeControl(item.id)}>
              <strong>{item.label}</strong>
          ...
          <div className="color-choice" role="group" aria-label="Choose your color">
            {(['random', 'white', 'black'] as const).map((color) => <button key={color} className={preferredColor === color ? 'active' : ''} onClick={() => setPreferredColor(color)}>{color === 'random' ? 'Random color' : `Play ${color}`}</button>)}
          </div>
          ...
          <button className="ghost wide" disabled={scanningRoom ||
                startingGame ||
                !recoveryChecked} onClick={() => void scanSenseRobotRoom()}>
            {scanningRoom
                ? 'Scanning…'
        ...
        {gameId && <section className="card game-card">
          ...
          <div className="move-list">
              {position.san.length === 0 ? (<span className="muted">Moves will appear here.</span>) : (Array.from({ length: Math.ceil(position.san.length / 2) }, (_, index) => {
                const whiteMoveIndex = index * 2;
                const blackMoveIndex = index * 2 + 1;
                const whitePly = whiteMoveIndex + 1;
                const blackPly = blackMoveIndex + 1;
                const whiteCoachNote = coachNotes.find((note) => note.ply === whitePly);
                const blackCoachNote = coachNotes.find((note) => note.ply === blackPly);
                const whiteMove = position.san[whiteMoveIndex];
                const blackMove = position.san[blackMoveIndex];
                return (<div className="move-row" key={index}>
                        <b>{index + 1}.</b>

                        {whiteMove ? (<button type="button" className={`move-cell ${historyPly === whitePly ? 'active' : ''} ${whiteCoachNote?.classification === 'blunder'
                            ? 'move-blunder'
                            : whiteCoachNote?.classification === 'mistake'
                                ? 'move-mistake'
                                : ''}`} onClick={() => {
                            setHistoryPly(whitePly >= position.plyCount
                                ? null
                                : whitePly);
                        }}>
                            {whiteMove}
                        ...
                        {blackMove ? (<button type="button" className={`move-cell ${historyPly === blackPly ? 'active' : ''} ${blackCoachNote?.classification === 'blunder'
                            ? 'move-blunder'
                            : blackCoachNote?.classification === 'mistake'
                                ? 'move-mistake'
                                : ''}`} onClick={() => {
                            setHistoryPly(blackPly >= position.plyCount
                                ? null
                                : blackPly);
                        }}>
                            {blackMove}
        ...
        <section className="card coach-card">
          ...
          <div className={`coach-bubble ${coachResult?.classification || ''}`}>
            {coachThinking ? <div className="coach-thinking"><span className="spinner"/>Analyzing your move…</div> : coachError ? <div className="inline-error">{coachError}</div> : coachResult ? <>
              <div className="coach-heading">
                ...
                {coachResult.shouldCoach ? (<span className={`quality-badge ${coachResult.classification}`}>
                    {coachResult.classification}
                  </span>) : null}
              </div>
              <div>{coachResult.feedback}</div>
              {coachResult.question ? (<div className="coach-question">
                  <span>
                    {coachLanguage === 'zh-CN'
                    ? '想一想'
                    : 'Ask yourself'}
              ...
              {coachResult.lesson ? (<div className="coach-lesson">
                {coachLanguage === 'zh-CN'
                    ? '记住：'
                    : 'Remember: '}
          ...
          <div className="coach-detail-setting">
            ...
            ['en', 'English'],
            ['zh-CN', '中文'],
        ] as const).map(([value, label]) => (<button key={value} type="button" className={`coach-detail-button ${coachLanguage === value
                ? 'active'
                : ''}`} aria-pressed={coachLanguage === value} onClick={() => {
                stopCoachSpeech();
                setCoachLanguage(value);
            }}>
                  <span>{label}</span>

                  {coachLanguage === value ? (<span className="coach-detail-check" aria-hidden="true">
                      ✓
                    </span>) : null}
                </button>))}
            </div>
          ...
          <div className="coach-detail-setting">
            ...
            ['balanced', 'Balanced'],
            ['deep', 'Deep'],
        ] as const).map(([value, label]) => (<button key={value} type="button" className={`coach-detail-button ${coachDetail === value ? 'active' : ''}`} aria-pressed={coachDetail === value} onClick={() => setCoachDetail(value)}>
                  <span>{label}</span>
                  {coachDetail === value ? (<span className="coach-detail-check" aria-hidden="true">✓</span>) : null}
                </button>))}
            </div>
          </div>
        ...
        {(gameId || coachNotes.length > 0) && <section className="card learning-card">
          ...
          {coachNotes.length === 0 ? <p className="muted">Your important coaching moments will collect here so you can review the exact position later.</p> : <div className="lesson-list">
            {orderedCoachNotes.map((note) => <button type="button" className="lesson-item" key={`${note.gameId}-${note.ply}`} onClick={() => { setReviewTarget(note); setReviewMode('better'); }}>
    ...
    {reviewMode && reviewTarget && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Review coached position" onMouseDown={(event) => { if (event.target === event.currentTarget)
            setReviewMode(null); }}>
      <div className="review-modal" style={{
                transform: `translate3d(${reviewDragX}px, 0, 0)`,
                transition: reviewNoTransition || reviewDragging
                    ? 'none'
                ...
                if (!reviewTouchStart.current ||
                    reviewAnimatingRef.current) {
                    return;
                }
                const touch = event.changedTouches[0];
                const dx = touch.clientX - reviewTouchStart.current.x;
                const dy = touch.clientY - reviewTouchStart.current.y;
        ...
        <div className="review-modal-head">
          ...
          <div className="review-nav">
            ...
            <button className="ghost" onClick={() => setReviewMode(null)}>
              Close
            </button>
          </div>
        </div>
        ...
        <div className="review-board-wrap">
          <ChessBoard fen={reviewMode === 'better' ? reviewTarget.fenBefore : reviewTarget.fenAfter} orientation={reviewOrientation} movableColor={undefined} destinations={new Map()} lastMove={reviewMode === 'threat' && reviewPlayedUci.length >= 4 ? [reviewPlayedUci.slice(0, 2), reviewPlayedUci.slice(2, 4)] as [
            string,
            string
        ] : undefined} coachArrows={reviewMode === 'better' && reviewBestUci.length >= 4
                ...
                : (reviewTarget.highlightsAfter || [])} rollbackSignal={0} onMove={() => undefined}/>
        </div>
        ...
        <div className="review-analysis">
          ...
          <div className="review-analysis-moves">
            ...
            {reviewTarget.opponentReply ? (<span>
                <small>Best reply</small>
                <strong>{reviewTarget.opponentReply}</strong>
              </span>) : null}
          </div>

          {reviewTarget.lesson ? (<div className="review-analysis-lesson">
              <span>Remember</span>
              {reviewTarget.lesson}
            </div>) : null}

          {reviewTarget.question ? (<div className="review-analysis-question">
    ...
    {gameOverOpen && terminalGame && (<div className="modal-backdrop game-over-backdrop" role="dialog" aria-modal="true" aria-label="Game report">
    <div className={`game-over-modal game-report-modal ${gameOutcomeClass}`}>
      ...
      {coachThinking ? (<div className="report-finishing">
      ...
      <div className="game-report-grid">
        <section className="report-card positive">
          ...
          {gameReport.strengths.map((strength, index) => (<p key={index}>
        ...
        <section className="report-card improve">
          ...
          {gameReport.improvements.map((improvement, index) => (<p key={index}>
                <span className="report-icon">
                  →
                </span>
                {improvement}
      ...
      <div className="practice-section">
        <div className="practice-heading">
          <div>
            ...
            <strong>
              Practice mistakes from this game
            </strong>
          </div>
        ...
        {practicePuzzles.length ? (<div className="practice-list">
            {practicePuzzles.map((puzzle, index) => (<button key={puzzle.ply} className="practice-card" onClick={() => openPuzzle(index)}>
          ...
          </div>) : (<p className="practice-empty">
      ...
      <div className="game-over-actions">
        <button className="ghost" onClick={() => setGameOverOpen(false)}>
          Keep board open
        </button>

        <button className="ghost" disabled={!orderedCoachNotes.length} onClick={() => {
        ...
        {practicePuzzles.length ? (<button className="primary" onClick={() => openPuzzle(0)}>
            Start personalized practice
          </button>) : (<button className="primary" onClick={resetFinishedGame}>
            New training game
          </button>)}

        {practicePuzzles.length ? (<button className="ghost full-width" onClick={resetFinishedGame}>
            New training game
          </button>) : null}
      </div>
    </div>
        ...
        {puzzleOpen && activePuzzle && (<div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Personalized puzzle">
    <div className="puzzle-modal">
      <div className="puzzle-head">
        ...
        <button className="ghost" onClick={closePuzzleToReport}>
          Close
        </button>
      </div>
      ...
      <div className="review-board-wrap">
        <ChessBoard fen={puzzleFen ||
                activePuzzle.fenBefore} orientation={activePuzzle.playerColor} movableColor={puzzleState === 'correct' ||
                puzzleState === 'revealed'
                ? undefined
                : puzzleMovableColor} destinations={puzzleState === 'correct' ||
                puzzleState === 'revealed'
                ? new Map()
                : puzzleDestinations} lastMove={undefined} coachArrows={puzzleState === 'revealed'
                ? [
                    {
                ...
                : []} coachHighlights={[]} rollbackSignal={puzzleRollbackSignal} onMove={handlePuzzleMove}/>
      </div>

      {puzzleState === 'solving' ? (<div className="puzzle-feedback neutral">
          Your move.
        </div>) : null}

      {puzzleState === 'incorrect' ? (<div className="puzzle-feedback wrong">
          Not quite. Look again for
          checks, captures, threats, and
          loose pieces.
        </div>) : null}

      {puzzleState === 'correct' ? (<div className="puzzle-feedback correct">
          <strong>
      ...
      {puzzleState === 'revealed' ? (<div className="puzzle-feedback revealed">
          <strong>
            Best move:{' '}
      ...
      <div className="puzzle-actions">
        {puzzleState !== 'correct' &&
                puzzleState !== 'revealed' ? (<button className="ghost" onClick={revealPuzzleAnswer}>
            Show answer
          </button>) : null}
        ...
        {(puzzleState === 'correct' ||
                puzzleState === 'revealed') &&
                puzzleIndex <
                    practicePuzzles.length - 1 ? (<button className="primary" onClick={() => openPuzzle(puzzleIndex + 1)}>
            Next puzzle
          </button>) : null}
        ...
        {(puzzleState === 'correct' ||
                puzzleState === 'revealed') &&
                puzzleIndex ===
                    practicePuzzles.length - 1 ? (<button className="primary" onClick={closePuzzleToReport}>
            Back to game report
          </button>) : null}
      </div>
    </div>
    ...
    {endGameConfirm && gameId && activeGame && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Confirm end game" onMouseDown={(event) => { if (event.target === event.currentTarget)
            setEndGameConfirm(false); }}>
